...
public class Autocomplete {
   ...
   public static class TrieAutocomplete implements Autocompletor {
      ...
      public Iterable<String> topMatches(String prefix, int k) {
         ...
         ArrayList<Term> match = new ArrayList<Term>();
         ...
         if (curr.isWord) {
            list.add(new Term(curr.myWord, curr.myWeight));
         }
         
         for (Node tree : curr.children.values()) {
            if (tree != null) {
               queue.offer(tree); 
            }
         }
         ...
         if (list.size() < k) {
            ...
            match.addAll(list.subList(0, k)); 
         }
         
         for (Term t : match) {
            finish.add(t.getWord()); 
         }
      ...
      public String topMatch(String prefix) {
         ...
         double weight = curr.mySubtreeMaxWeight;
         
         if (curr.myWeight == weight) {
            return curr.getWord(); 
         }
            
         while (curr.getWeight() != weight) {
            for (Node tree : curr.children.values()) {
               if (tree.mySubtreeMaxWeight == weight) {
                  curr = tree;
                  break;
               }
            }
